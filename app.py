...
def load_tokens(server_name, for_visit=False):
    try:
        ...
        return []
            ...
            for item in data:
                ...
                if "token" in item:
                    ...
                    token_value = item.get("password", "")
                elif "access_token" in item:
                    token_value = item.get("access_token", "")
                else:
                    # If no token field found, skip this item
                    print(f"Warning: Item in {path} has no recognizable token field")
                    continue
                
                if token_value:  # Only add if token_value is not empty
                    processed_tokens.append({
                        "uid": uid_value,
                        "token": token_value
                    })
            
            print(f"Loaded {len(processed_tokens)} tokens from {path} for server {server_name}")
            return processed_tokens
        ...
        print(f"Unexpected error loading tokens from {path}: {e}")
        return []
...
def make_profile_check_request(encrypted_profile_payload, server_name, token_dict):
    token_value = token_dict.get("token", "") if token_dict else ""
    if not token_value:
...
# হোম পেজ রুট - HTML ফাইল সার্ভ করার জন্য
@app.route('/')
def serve_html():
